...
复刻 App QemuArgsBuilder 的参数形态启动 QEMU，验证三件事：
  1) 分辨率注入：-device virtio-gpu-device,xres=,yres= → guest virtio-gpu
     驱动经 GET_DISPLAY_INFO 跟随宿主 → VNC 初始帧缓冲必须等于 xres/yres
...
for attempt in range(6):
    try:
        v = Vnc()
        log(f"VNC 握手初始帧缓冲: {v.w}x{v.h} (期望 {args.xres}x{args.yres})")
...
ok_size = (fb_w == args.xres and fb_h == args.yres)
if frameA is None:
    log("显示验证失败: 无非黑帧"); print(open(LOG).read()[-1500:]); sys.exit(4)
if not ok_size:
    log(f"FAIL: 帧缓冲 {fb_w}x{fb_h} != 注入 {args.xres}x{args.yres} —— 分辨率注入未生效(椭圆根因)")
    sys.exit(5)
log(">>> 分辨率注入 PASS")
log(">>> 画面渲染 PASS")
